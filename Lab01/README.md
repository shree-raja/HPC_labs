# Lab 01: Performance Profiling and Optimization using GPROF and PERF

## Overview

Investigate an application's performance with nearly 50 years of daily temperature data (≈18,250 records), identify bottlenecks, analyze the collected data, and recommend suitable optimizations using Linux performance analysis tools.

## Objectives
- Implement and compare different sorting algorithms.
- Measure execution time using Linux timing tools.
- Profile applications using GPROF.
- Collect hardware performance statistics using PERF.
- Analyze the impact of compiler optimizations.
- Study algorithm performance with increasing input sizes.
- Evaluate the possibility of parallel execution.

## Results
Quick Sort performed better than Bubble Sort due to its lower time complexity. Compiler optimizations improved execution efficiency by generating more optimized code.
